/**
 * Submit Supervisor Evaluation (Company) use-case
 *
 * A company supervisor submits the 5th evaluation criterion (/4) for a PFE
 * they supervise. Stored in supervisor_evaluations.
 */
...
export async function submitSupervisorEvaluation(
    ...
    if (admins.length > 0) {
        await sendNotification(repos, {
            ...
            emailBody: `<p>L'encadrant entreprise a soumis son évaluation (critère 5) pour le PFE.</p>`,
        });
    }
}